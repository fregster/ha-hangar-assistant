...
def get_aircraft_airfield(aircraft_config: dict, hangars: list, airfields: list) -> dict | None:
...
def get_airfield_for_hangar(hangar_config: dict, airfields: list) -> dict | None:
...
def get_hangar_sensor_value(
    ...
    """
    # Priority 1: Hangar sensor
    if hangar_config and (sensor_id := hangar_config.get(sensor_type)):
        state = hass.states.get(sensor_id)
        if state and state.state not in ["unavailable", "unknown", "none", ""]:
            try:
                return float(state.state)
            except (ValueError, TypeError):
                _LOGGER.debug(f"Hangar sensor {sensor_id} has non-numeric value: {state.state}")
    
    # Priority 2: Airfield sensor
    if airfield_config and (sensor_id := airfield_config.get(sensor_type)):
        state = hass.states.get(sensor_id)
        if state and state.state not in ["unavailable", "unknown", "none", ""]:
            try:
                return float(state.state)
            except (ValueError, TypeError):
                _LOGGER.debug(f"Airfield sensor {sensor_id} has non-numeric value: {state.state}")
    
    # Priority 3: Global sensor
    if global_sensor:
        state = hass.states.get(global_sensor)
        if state and state.state not in ["unavailable", "unknown", "none", ""]:
            try:
                return float(state.state)
            except (ValueError, TypeError):
                _LOGGER.debug(f"Global sensor {global_sensor} has non-numeric value: {state.state}")
...
def get_hangar_temperature(
    ...
    if aircraft_config and not hangar_config and not airfield_config:
        ...
        if isinstance(hass, HomeAssistant):
            ...
            airfield_config = get_aircraft_airfield(aircraft_config, hangars, airfields)
